...
Orange %>%
  group_by(Tree) %>%
  filter(age == max(age)) %>%
  summarise(growth_rate = mean(circumference / age))
```
...
This dplyr pipe is used to calculate the average growth rates for each tree,
ranging from age 0 to the final measured age. Each row repreents the average
growth rate from the previous time point and the standard error in this time
period.
