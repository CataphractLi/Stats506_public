...
if (!file.exists(cbecs_clean)) {
  cbecs <- read_delim("2012_public_use_data_aug2016.csv", delim = ",") %>%
    select(PUBID, CENDIV, PBA, SQFT, WKHRS, 
           ZWKHRS, WKHRSC, ZWKHRSC, FINALWT) %>%
    filter(ZWKHRS == 0, ZWKHRSC == 0) %>% # Delete the imputed observations
    select(-ZWKHRS, -ZWKHRSC) %>%
...
for (var in c("CENDIV", "PBA", "WKHRSC")) {
  tibble <- codebook %>%
    filter(Variable == var)
...
wh_proportion <- cbecs %>%
  ...
  summarise(Weights = sum(FINALWT, na.rm = T), .groups = "drop") %>%
  group_by(CENDIV) %>%
...
wh_proportion_plot <- wh_proportion_variance %>%
  ggplot(aes(x = Proportions, y = PBA, 
             color = WKHRSC)) +
  geom_point(
    position = position_dodge2(width = 0.5)
  ) +
